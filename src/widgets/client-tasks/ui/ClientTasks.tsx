...
export function ClientTasks({ tasks, onAddTask, onUpdateTask, onDeleteTask }: ClientTasksProps) {
  ...
  const renderTask = (task: Task) => {
    ...
    return (
      <div key={task.id} className={`bg-white border rounded-xl p-4 flex items-start gap-3 group ${
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
          ...
          <button onClick={() => onDeleteTask(task.id)} className="p-1 text-slate-300 hover:text-red-400 transition-colors">
            <Trash2 size={13} />
          </button>
        </div>
      </div>
